...
if(isset($_GET['getmsg']) AND !empty($_GET['getmsg'])) {
	...
	*/} elseif ($getmsg == "json") {
		...
		for($i=0; $i<count($result); $i++) {
			...
			echo '{"id":"'.$result[$i]["id"].'", content":"'.str_replace('"', '\\"',$result[$i]["content"]).'", "sender":"'.$result[$i]["sender"].'", "date_time":"'.$result[$i]["date_time"].'", "id_certified_user":"'.$result[$i]["id_certified_user"].'", "rank":"'.$r.'"}';
			if($i < (count($result)-1)) {
				echo ",";
			}
		}
